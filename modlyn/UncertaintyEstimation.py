...
class ProperUncertaintyEstimator:
    def __init__(self, model, adata, datamodule):
        ...
        self.feature_names = getattr(adata.var, "gene_symbols", None) or [
            f"Feature_{i}" for i in range(self.weights.shape[1])
        ]
    ...
    def method_1_fisher_information(self):
        ...
        for k in range(n_classes):
            for j in range(n_features):
                fisher_diag = np.sum(
                    all_probs[:, k] * (1 - all_probs[:, k]) * (all_x[:, j] ** 2)
                )
                if fisher_diag > 1e-8:
    ...
    def method_3_bootstrap_proper(self, n_bootstrap=30):
        ...
        if len(bootstrap_weights) > 10:
            ...
            print(
                f" Bootstrap completed with {len(bootstrap_weights)} successful iterations"
            )
    ...
    def plot_uncertainty_overview(self, results):
        ...
        axes[0, 0].axvline(
            ...
            color="red",
            linestyle="--",
            label=f"Median: {np.median(se_flat):.3f}",
        )
        axes[0, 0].legend()
        ...
        axes[0, 1].hist(
            z_flat, bins=50, alpha=0.7, color="lightgreen", edgecolor="black"
        )
        ...
        axes[0, 1].axvline(
            1.96, color="orange", linestyle="--", alpha=0.7, label="p=0.05"
        )
        ...
        valid_idx = np.isfinite(weights_flat) & np.isfinite(
            se_flat[: len(weights_flat)]
        )
        scatter = axes[1, 0].scatter(
            np.abs(weights_flat[valid_idx]),
            se_flat[valid_idx],
            alpha=0.6,
            c=p_flat[valid_idx],
            cmap="RdYlGn_r",
        ...
        counts = [
            np.sum(p_flat < 0.001),
            np.sum((p_flat >= 0.001) & (p_flat < 0.01)),
            np.sum((p_flat >= 0.01) & (p_flat < 0.05)),
            np.sum(p_flat >= 0.05),
        ]
    ...
    def plot_uncertainty_heatmap(self, results, top_n_features=30):
        ...
        fig.suptitle(
            f"Uncertainty Heatmaps (Top {top_n_features} Most Uncertain Features)",
            fontsize=16,
        )

        selected_features = [
            self.feature_names[i][:15] + "..."
        ...
        im2 = axes[1].imshow(
            p_selected, cmap="RdYlGn_r", aspect="auto", vmin=0, vmax=0.1
        )
    ...
    def plot_feature_importance_with_uncertainty(self, results, top_n=15):
        ...
        axes[0].set_yticklabels(
            [self.feature_names[i] for i in top_features_idx], fontsize=9
        )
    ...
    def create_dashboard(self, results):
        ...
        p_flat = p_flat[np.isfinite(p_flat)]
        axes[0, 1].hist(
            p_flat, bins=30, alpha=0.7, color="lightblue", edgecolor="black"
        )
        axes[0, 1].axvline(
            0.05, color="red", linestyle="--", linewidth=2, label="α = 0.05"
        )
...
Model Shape: {self.weights.shape[0]} classes × {self.weights.shape[1]} features"""
    ...
    def compare_methods(self):
        ...
        try:
            se_bootstrap, bootstrap_weights = self.method_3_bootstrap_proper(
                n_bootstrap=20
            )
        ...
        print(
            f"Significant weights (p < 0.05): {np.sum(p_values < 0.05)} / {p_values.size}"
        )
...
def get_proper_uncertainty(
    model, adata, datamodule, create_visualizations=True, interactive=False
):
    ...
    if create_visualizations:
        ...
        importance_fig.savefig(
            "feature_importance_uncertainty.png", dpi=300, bbox_inches="tight"
        )
