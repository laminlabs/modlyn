...
class GeneExpressionAnalyzer:
    ...
    def __init__(self, adata, subsample_size=100000):
        ...
        if self.adata.n_obs > subsample_size:
            print(
                f" Subsampling {subsample_size:,} cells from {self.adata.n_obs:,} for analysis..."
            )
    ...
    def _extract_perturbations(self):
        ...
        pert_cols = [
            ...
            if any(
                x in col.lower()
                for x in ["drug", "treatment", "perturbation", "compound"]
            )
        ]
    ...
    def figure_1_expression_overview(self):
        ...
        if hasattr(self.adata, "X"):
            ...
            if len(expression_data) > 1000000:
                expression_data = np.random.choice(
                    expression_data, 1000000, replace=False
                )

            axes[0, 0].hist(
                expression_data, bins=50, alpha=0.7, color="skyblue", edgecolor="black"
            )
        ...
        if hasattr(self.adata.obs, "total_counts"):
            axes[0, 2].hist(
                self.adata.obs["total_counts"], bins=50, alpha=0.7, color="salmon"
            )
        ...
        if len(self.perturbations) > 1:
            ...
            if pert_col:
                ...
                axes[1, 0].set_yticklabels(
                    [
                        str(p)[:15] + "..." if len(str(p)) > 15 else str(p)
                        for p in top_perts.index
                    ],
                    fontsize=6,
                )
        ...
        if self.cell_lines:
            ...
            if cell_col:
                ...
                axes[1, 1].pie(
                    top_lines.values,
                    labels=[
                        str(l)[:10] + "..." if len(str(l)) > 10 else str(l)
...
"""
        ...
        axes[1, 2].text(
            ...
            bbox={"boxstyle": "round,pad=0.3", "facecolor": "#F0F8FF", "alpha": 0.8},
        )

        plt.suptitle(
            "Gene Expression Overview and Quality Control",
            fontsize=16,
            fontweight="bold",
        )
        plt.tight_layout()
    ...
    def figure_2_differential_expression(self):
        ...
        if len(self.perturbations) >= 2:
            ...
            if np.sum(mask1) > 10 and np.sum(mask2) > 10:
                ...
                significance = np.abs(log_fc) + np.random.normal(
                    0, 0.5, len(log_fc)
                )  # Mock p-values
        ...
        if "differential" in self.results:
            ...
            axes[0, 1].set_yticklabels(
                [self.gene_names[i] for i in top_up_idx], fontsize=6
            )
        ...
        if "differential" in self.results:
            ...
            axes[0, 2].set_yticklabels(
                [self.gene_names[i] for i in top_down_idx], fontsize=6
            )
        ...
        perturbation_subset = (
            ...
            if len(self.perturbations) > 10
            else self.perturbations
        )

        # Mock enrichment scores
        ###### !!!! REPLACE THIS #####
        enrichment_matrix = np.random.randn(
            len(pathway_names), len(perturbation_subset)
        )
        ...
        axes[1, 0].set_xticklabels(
            [
                str(p)[:10] + "..." if len(str(p)) > 10 else str(p)
                for p in perturbation_subset
            ],
            rotation=45,
            ha="right",
            fontsize=7,
        ...
        if gene_cv is not None:
            axes[1, 1].hist(
                gene_cv, bins=50, alpha=0.7, color="purple", edgecolor="black"
            )
...
all perturbations identified."""
    ...
    def _calculate_gene_variability(self):
        ...
        try:
            ...
            if len(perturbation_means) > 2:
                ...
                gene_cv = np.std(perturbation_means, axis=0) / (
                    np.mean(perturbation_means, axis=0) + 1e-6
                )
    ...
    def figure_3_cell_clustering_analysis(self):
        ...
        if pert_col and pert_col in self.adata.obs.columns:
            ...
            unique_perts = self.adata.obs[pert_col].unique()[
                :20
            ]  # Top 20 for visibility

            for _i, pert in enumerate(unique_perts):
                mask = self.adata.obs[pert_col] == pert
                if np.sum(mask) > 0:
                    axes[0, 0].scatter(
                        self.adata.obsm["X_pca"][mask, 0],
        ...
        if cell_col and cell_col in self.adata.obs.columns:
            unique_lines = self.adata.obs[cell_col].unique()[
                :10
            ]  # Top 10 for visibility

            for _i, line in enumerate(unique_lines):
        ...
        axes[0, 2].bar(
            range(1, len(var_ratio) + 1), var_ratio, alpha=0.7, color="green"
        )
        ...
        try:
            ...
            if pert_col and pert_col in self.adata.obs.columns:
                ...
                axes[1, 0].set_title(
                    "D. UMAP: Colored by Perturbation", fontweight="bold"
                )
                axes[1, 0].legend(
...
reveals distinct populations."""
        ...
        axes[1, 2].text(
            ...
            va="top",
            fontfamily="monospace",
            bbox={"boxstyle": "round,pad=0.3", "facecolor": "#E6E6FA", "alpha": 0.8},
        )

        plt.suptitle(
            "Cell Type and Perturbation Clustering Analysis",
            fontsize=16,
            fontweight="bold",
        )
        plt.tight_layout()
    ...
    def figure_4_drug_response_analysis(self):
        ...
        for _, drug in enumerate(self.perturbations[:5]):  # Show 5 drugs
            ...
            axes[0, 1].semilogx(
                doses, response, "o-", label=str(drug)[:10], linewidth=2, markersize=4
            )
        ...
        if "drug_similarity" in self.results:
            ...
            try:
                ...
                dendrogram(
                    ...
                    labels=[
                        str(p)[:8]
                        for p in self.perturbations[: len(linkage_matrix) + 1]
                    ],
                    leaf_rotation=90,
                    leaf_font_size=6,
                )
        ...
        if self.cell_lines and len(self.cell_lines) > 1:
            ...
            if sensitivity_data is not None:
                ...
                axes[1, 0].set_xticklabels(
                    [str(p)[:8] for p in self.perturbations[:n_drugs_show]],
                    rotation=45,
                    ha="right",
                    fontsize=6,
                )
                ...
                axes[1, 0].set_yticklabels(
                    [str(l)[:8] for l in self.cell_lines[:n_lines_show]], fontsize=6
                )
        ...
        axes[1, 1].text(
            ...
            va="top",
            fontfamily="monospace",
            bbox={"boxstyle": "round,pad=0.3", "facecolor": "#FFE4E1", "alpha": 0.8},
        )
...
"""
        ...
        axes[1, 2].text(
            ...
            va="top",
            fontfamily="monospace",
            bbox={"boxstyle": "round,pad=0.3", "facecolor": "#F0FFFF", "alpha": 0.8},
        )

        plt.suptitle(
            "Drug Response and Mechanism Analysis", fontsize=16, fontweight="bold"
        )
        plt.tight_layout()
    ...
    def _calculate_drug_similarity(self):
        ...
        try:
            ...
            for pert in self.perturbations[
                :20
            ]:  # Limit to first 20 for computational efficiency
                mask = self.adata.obs[pert_col] == pert
        ...
        if hasattr(self.adata, "var"):
            ...
            if "highly_variable" not in self.adata.var.columns:
                try:
                    ...
                    self.adata.var.iloc[
                        top_var_idx, self.adata.var.columns.get_loc("highly_variable")
                    ] = True

            # Get highly variable genes
            if "highly_variable" in self.adata.var.columns:
                hvg_genes = self.adata.var_names[
            ...
            if pert_col and len(hvg_genes) > 0:
                try:
                    ...
                    ax_dotplot.set_title(
                        "A. Gene Expression Dotplot (Top Variable Genes)",
                        fontweight="bold",
                        pad=20,
                    )
                except Exception as e:
                    print(f"Scanpy dotplot failed: {e}, creating custom plot...")
                    self._create_custom_expression_dotplot(
                        ax_dotplot, hvg_genes, pert_col
                    )
            else:
                ax_dotplot.text(
                    0.5,
                    ...
                    ha="center",
                    va="center",
                    transform=ax_dotplot.transAxes,
                )
                ax_dotplot.set_title(
                    "A. Expression Dotplot (Not Available)", fontweight="bold"
                )
...
"""
        ...
        if "differential" in self.results:
            ...
            ax_heatmap.set_yticklabels(
                [self.gene_names[i] for i in top_genes_idx], fontsize=6
            )
            ax_heatmap.set_xticks(range(n_perts_show))
            ax_heatmap.set_xticklabels(
                [str(p)[:8] for p in self.perturbations[:n_perts_show]],
                rotation=45,
                ha="right",
                fontsize=6,
            )
        ...
        colors = [
            "darkred" if p < 0.01 else "red" if p < 0.05 else "orange" for p in p_values
        ]

        bars = ax_pathway.barh(
            range(len(pathways)), enrichment_scores, color=colors, alpha=0.7
        )
        ...
        for _i, (bar, p_val) in enumerate(zip(bars, p_values)):
            significance = (
                "***"
                if p_val < 0.001
                else "**"
...
clinical translation."""
        ...
        ax_summary.text(
            ...
            va="top",
            fontfamily="monospace",
            bbox={"boxstyle": "round,pad=0.3", "facecolor": "#FFFACD", "alpha": 0.8},
        )

        plt.suptitle(
            "Gene Expression Analysis and Biological Insights",
            fontsize=16,
            fontweight="bold",
        )
...
def main():
    ...
    try:
        ...
        adata.obs["y"] = np.random.choice(
            ["Drug_A", "Drug_B", "Drug_C", "Control"], n_cells
        )
