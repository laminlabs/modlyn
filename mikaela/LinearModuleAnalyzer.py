...
class LinearModelAnalyzer:
    ...
    def _extract_gene_names(self, adata):
        ...
        if adata.var.shape[1] > 0:
            ...
            gene_columns = ['feature_name', 'gene_name', 'symbol', 'gene_symbol', 'gene_id']
            ...
            if len(adata.var.columns) > 0:
                ...
                if any(any(c.isalpha() for c in val) and len(val) > 1 for val in sample_vals):
    ...
    def create_custom_scanpy_dotplot(self, top_k=20, use_abs_size=True):
        ...
        fig, ax = plt.subplots(figsize=(max(12, len(top_gene_names) * 0.4), 
                                       max(8, max_classes * 0.3)))
        ...
        for i, class_name in enumerate(weight_pivot.index):
            for j, gene_name in enumerate(weight_pivot.columns):
                ...
                ax.scatter(j, i, s=size, c=weight, cmap='RdBu_r', 
        ...
        sm = plt.cm.ScalarMappable(cmap='RdBu_r', 
                                  norm=plt.Normalize(vmin=-abs(weight_pivot.values).max(), 
                                                   vmax=abs(weight_pivot.values).max()))
        sm.set_array([])
...
def quick_analysis_with_scanpy_dotplot(model, adata, datamodule=None):
    ...
    print("\n heatmap...")
    top_genes = analyzer.create_heatmap_analysis(top_k=20)
