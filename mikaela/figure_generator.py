...
class FigureGenerator:
    ...
    def create_figure1_method_comparison(self, cell_line=None, n_top_genes=20):
        ...
        if cell_line is None:
            available_lines = [cl for cl in results["scanpy"].keys() 
        ...
        if cell_line in results["scanpy"] and not results["scanpy"][cell_line].empty:
            ...
            bars1 = axes[0].barh(y_pos, scanpy_data["scores"], 
                               color=colors["scanpy"], alpha=0.8)
            axes[0].set_yticks(y_pos)
            axes[0].set_yticklabels(scanpy_data["names"], fontsize=10)
            axes[0].set_xlabel("Wilcoxon Score", fontsize=14, fontweight='bold')
            axes[0].set_title("Scanpy\n(Statistical DE)", fontsize=16, fontweight='bold')
            axes[0].grid(axis='x', alpha=0.3)
            
            # Add value labels for top 5
            for i, (idx, row) in enumerate(scanpy_data.head(5).iterrows()):
                axes[0].text(row["scores"] + 0.02 * max(scanpy_data["scores"]), i, 
                           f'{row["scores"]:.1f}', va='center', fontsize=9, fontweight='bold')
        else:
            axes[0].text(0.5, 0.5, "Scanpy\nNo Results", ha='center', va='center',
                        transform=axes[0].transAxes, fontsize=16, fontweight='bold')
        
        # 2. LinearSCVI (Middle)
        if (results["linscvi"] and cell_line in results["linscvi"] and 
            not results["linscvi"][cell_line].empty):
            linscvi_data = results["linscvi"][cell_line].sort_values("lfc_median", ascending=False).head(n_top_genes)
            y_pos = np.arange(len(linscvi_data))
            
            # Color by positive/negative LFC
            colors_lfc = [colors["linscvi"] if lfc > 0 else '#3498db' 
                         for lfc in linscvi_data["lfc_median"]]
            
            bars2 = axes[1].barh(y_pos, linscvi_data["lfc_median"], 
                               color=colors_lfc, alpha=0.8)
            axes[1].set_yticks(y_pos)
            axes[1].set_yticklabels(linscvi_data.index, fontsize=10)
            axes[1].set_xlabel("Log Fold Change", fontsize=14, fontweight='bold')
            axes[1].set_title("LinearSCVI\n(Variational DE)", fontsize=16, fontweight='bold')
            axes[1].grid(axis='x', alpha=0.3)
            axes[1].axvline(x=0, color='black', linestyle='-', alpha=0.5)
            
            # Add value labels for top 5
            for i, (gene, row) in enumerate(linscvi_data.head(5).iterrows()):
                axes[1].text(row["lfc_median"] + 0.02 * max(abs(linscvi_data["lfc_median"])), i,
                           f'{row["lfc_median"]:.2f}', va='center', fontsize=9, fontweight='bold')
        else:
            axes[1].text(0.5, 0.5, "LinearSCVI\nNot Available", ha='center', va='center',
        ...
        colors_weight = [colors["modlyn"] if w > 0 else '#e74c3c' 
                        for w in modlyn_data["weight"]]
        
        bars3 = axes[2].barh(y_pos, modlyn_data["weight"], 
                           color=colors_weight, alpha=0.8)
        ...
        # Add value labels for top 5
        for i, (idx, row) in enumerate(modlyn_data.head(5).iterrows()):
            axes[2].text(row["weight"] + 0.02 * max(abs(modlyn_data["weight"])), i,
                       f'{row["weight"]:.3f}', va='center', fontsize=9, fontweight='bold')
        
        # Overall styling
        fig.suptitle(f'Top {n_top_genes} Marker Genes for {cell_line}', 
        ...
        plt.savefig(output_path.with_suffix('.pdf'), format='pdf', bbox_inches='tight', facecolor='white')
    ...
    def create_figure2_volcano_plots(self, cell_line=None):
        ...
        if cell_line is None:
            available_lines = [cl for cl in results["scanpy"].keys() 
                             if not results["scanpy"][cl].empty]
            cell_line = available_lines[0] if available_lines else list(results["modlyn"].keys())[0]
        ...
        if cell_line in results["scanpy"] and not results["scanpy"][cell_line].empty:
            ...
            if not scanpy_data.empty and 'pvals' in scanpy_data.columns:
                ...
                axes[0].scatter(x[~significant], y[~significant], 
                              alpha=0.6, s=20, color='lightgray', label='Not significant')
                axes[0].scatter(x[significant], y[significant], 
                              alpha=0.8, s=20, color='#3498db', label='Significant')
        ...
        if (results["linscvi"] and cell_line in results["linscvi"] and 
            ...
            if not linscvi_data.empty:
                ...
                axes[1].scatter(x[~significant], y[~significant], 
                              alpha=0.6, s=20, color='lightgray', label='Not significant')
                axes[1].scatter(x[significant], y[significant], 
                              alpha=0.8, s=20, color='#e74c3c', label='Significant')
                
                axes[1].set_xlabel("Log Fold Change", fontsize=12)
                axes[1].set_ylabel("-log10(prob not DE)", fontsize=12)
                axes[1].set_title("LinearSCVI Volcano Plot", fontsize=14, fontweight='bold')
                axes[1].legend()
        ...
        axes[2].scatter(x[~significant], y[~significant], 
                      alpha=0.6, s=20, color='lightgray', label='Not significant')
        axes[2].scatter(x[significant], y[significant], 
                      alpha=0.8, s=20, color='#2ecc71', label='Significant')
        ...
        fig.suptitle(f'Statistical Significance Comparison - {cell_line}', 
    ...
    def create_figure3_concordance_analysis(self):
        ...
        concordance_df = pd.read_csv(self.analysis.tables_dir / "biological_concordance.csv")
        
        fig, axes = plt.subplots(2, 2, figsize=(16, 12))
        
        # 1. Jaccard similarity heatmap
        jaccard_data = concordance_df[['scanpy_modlyn_jaccard', 'scanpy_linscvi_jaccard', 'modlyn_linscvi_jaccard']]
        jaccard_matrix = jaccard_data.mean().values.reshape(1, 3)
        
        im1 = axes[0,0].imshow(jaccard_matrix, cmap='YlOrRd', aspect='auto', vmin=0, vmax=1)
        axes[0,0].set_xticks([0, 1, 2])
        axes[0,0].set_xticklabels(['Scanpy-MODLYN', 'Scanpy-LinearSCVI', 'MODLYN-LinearSCVI'], rotation=45)
        ...
        for j in range(3):
            text = axes[0,0].text(j, 0, f'{jaccard_matrix[0, j]:.3f}', 
                                ha="center", va="center", color="black", fontweight='bold')
        
        plt.colorbar(im1, ax=axes[0,0])
        
        # 2. Distribution of overlaps
        axes[0,1].hist(concordance_df['scanpy_modlyn_jaccard'], alpha=0.7, label='Scanpy-MODLYN', bins=15)
        axes[0,1].hist(concordance_df['modlyn_linscvi_jaccard'], alpha=0.7, label='MODLYN-LinearSCVI', bins=15)
        ...
        # 3. Three-way overlap
        axes[1,0].bar(range(len(concordance_df)), concordance_df['three_way_overlap'], 
        ...
        method_counts = concordance_df[['n_scanpy_genes', 'n_modlyn_genes', 'n_linscvi_genes']].mean()
        axes[1,1].bar(['Scanpy', 'MODLYN', 'LinearSCVI'], method_counts, 
        ...
        for i, v in enumerate(method_counts):
            axes[1,1].text(i, v + 0.5, f'{v:.0f}', ha='center', va='bottom', fontweight='bold')
    ...
    def create_figure4_performance_benchmarks(self):
        ...
        for bar, mem_val in zip(bars2, memory_usage):
            ...
            axes[1].text(bar.get_x() + bar.get_width()/2., height + 10,
                       f'{mem_val:.0f}MB', ha='center', va='bottom', fontweight='bold')
        ...
        for bar, eff_val in zip(bars3, efficiency):
            ...
            axes[2].text(bar.get_x() + bar.get_width()/2., height + 5,
                       f'{eff_val:.0f}', ha='center', va='bottom', fontweight='bold')
    ...
    def create_figure5_scalability_analysis(self):
        ...
        """
        try:
            scalability_df = pd.read_csv(self.analysis.tables_dir / "scalability_analysis.csv")
        ...
        for method in scalability_df['method'].unique():
            ...
            axes[0,0].plot(method_data['n_cells'], method_data['runtime_seconds'], 
                          'o-', label=method.title(), color=color, linewidth=2, markersize=8)
        ...
        for method in scalability_df['method'].unique():
            method_data = scalability_df[scalability_df['method'] == method]
            color = '#3498db' if method == 'scanpy' else '#2ecc71'
            axes[0,1].plot(method_data['n_cells'], method_data['memory_mb'], 
                          'o-', label=method.title(), color=color, linewidth=2, markersize=8)
        ...
        for method in scalability_df['method'].unique():
            method_data = scalability_df[scalability_df['method'] == method]
            color = '#3498db' if method == 'scanpy' else '#2ecc71'
            axes[1,0].plot(method_data['n_cells'], method_data['cells_per_second'], 
                          'o-', label=method.title(), color=color, linewidth=2, markersize=8)
        ...
        if len(scanpy_data) == len(modlyn_data):
            speedup = scanpy_data['runtime_seconds'].values / modlyn_data['runtime_seconds'].values
            ...
            for i, v in enumerate(speedup):
                axes[1,1].text(i, v + 0.1, f'{v:.1f}x', ha='center', va='bottom', fontweight='bold')
    ...
    def create_venn_diagram(self, cell_line=None):
        ...
        if (results["linscvi"] and cell_line in results["linscvi"] and 
            not results["linscvi"][cell_line].empty):
            linscvi_top = results["linscvi"][cell_line].sort_values("lfc_median", ascending=False)
        ...
        if len(linscvi_genes) > 0:
            venn = venn3([scanpy_genes, modlyn_genes, linscvi_genes],
            ...
            venn = venn2([scanpy_genes, modlyn_genes],
    ...
    def create_correlation_heatmap(self):
        ...
        for cell_line in results["modlyn"].keys():
            if (cell_line in results["scanpy"] and 
                not results["scanpy"][cell_line].empty):
                ...
                if len(common_genes) > 10:  # Need sufficient overlap
                    ...
                    corr, p_val = spearmanr(scanpy_subset["scores"], modlyn_subset["abs_weight"])
                    correlations.append({
                        "cell_line": cell_line,
                        "correlation": corr,
                        "p_value": p_val,
        ...
        if correlations:
            ...
            # Create heatmap
            corr_matrix = corr_df.set_index("cell_line")["correlation"].values.reshape(-1, 1)
            ...
            for i, corr_val in enumerate(corr_matrix[:, 0]):
                ax.text(0, i, f'{corr_val:.3f}', ha="center", va="center", 
    ...
    def create_robustness_analysis(self):
        ...
        # Plot distributions
        axes[0].hist(modlyn_weights, bins=30, alpha=0.7, label='MODLYN weights', color='#2ecc71')
        if scanpy_scores:
            axes[0].hist(scanpy_scores, bins=30, alpha=0.7, label='Scanpy scores', color='#3498db')
        ...
        for method in methods:
            if method == 'MODLYN':
                top_genes_per_line = [len(results["modlyn"][cl].head(50)) for cl in results["modlyn"].keys()]
            else:
                top_genes_per_line = [len(results["scanpy"][cl].head(50)) 
                                    for cl in results["scanpy"].keys() 
                                    if not results["scanpy"][cl].empty]
            
            cv = np.std(top_genes_per_line) / np.mean(top_genes_per_line) if top_genes_per_line else 0
