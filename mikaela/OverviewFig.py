"""
MODLYN Nature Figure: Scalable Linear Model Analysis Framework
============================================================
...
class MODLYNFigure:
    ...
    def create_comprehensive_figure(self, model=None, adata=None):
        ...
        gs = GridSpec(4, 4, figure=fig, hspace=0.3, wspace=0.3,
                     height_ratios=[1, 1.2, 1, 1], width_ratios=[1, 1, 1, 1])
        ...
        # Add main title
        fig.suptitle('MODLYN: Linear Models for Massive Single-Cell Analysis', 
    ...
    def _draw_framework_overview(self, ax):
        ...
        # Draw workflow boxes
        boxes = [
            {'xy': (0.5, 1), 'width': 1.5, 'height': 0.8, 'label': 'Tahoe-100M\nDataset\n(200M cells)', 'color': self.colors['data']},
            {'xy': (2.5, 1), 'width': 1.5, 'height': 0.8, 'label': 'MODLYN\nData Loader', 'color': self.colors['model']},
            {'xy': (4.5, 1), 'width': 1.5, 'height': 0.8, 'label': 'Linear\nClassifier', 'color': self.colors['model']},
            {'xy': (6.5, 1), 'width': 1.5, 'height': 0.8, 'label': 'Weight\nAnalysis', 'color': self.colors['analysis']},
            {'xy': (8.5, 1), 'width': 1.2, 'height': 0.8, 'label': 'Biological\nInsights', 'color': self.colors['biology']}
        ]
        ...
        # Add subtitle
        ax.text(5, 0.2, '"Linear models in the age of AI"', 
               ha='center', va='center', fontsize=10, style='italic',
               bbox=dict(boxstyle="round,pad=0.3", facecolor=self.colors['accent'], alpha=0.5))
    ...
    def _draw_scale_comparison(self, ax):
        ...
        colors_scale = [self.colors['accent'], self.colors['data'], self.colors['model'], self.colors['biology']]
        
        bars = ax.bar(datasets, cell_counts, color=colors_scale, alpha=0.8, edgecolor='black')
        ax.set_yscale('log')
        ...
        for bar, count in zip(bars, cell_counts):
            ...
            ax.text(bar.get_x() + bar.get_width()/2., height*1.1,
                   f'{count:.0e}', ha='center', va='bottom', fontsize=7)
    ...
    def _draw_model_architecture(self, ax):
        ...
        for i, y in enumerate(gene_y_positions):
            ...
            ax.add_patch(circle)
            if i == 2:  # Middle gene
                ax.text(2, y, 'Gene\n19K', ha='center', va='center', fontsize=6, fontweight='bold')
        
        # Add "..." to indicate more genes
        ax.text(2, 5.5, '...', ha='center', va='center', fontsize=12, fontweight='bold')
        
        # Weight matrix
        ax.add_patch(plt.Rectangle((4, 3), 2, 4, facecolor=self.colors['model'], alpha=0.7, edgecolor='black'))
        ax.text(5, 5, 'Weight\nMatrix\n19K×50', ha='center', va='center', fontsize=8, fontweight='bold', color='white')
        ...
        for i, y in enumerate(class_y_positions):
            ...
            ax.add_patch(circle)
            if i == 1:  # Second class
                ax.text(8, y, 'Class\n50', ha='center', va='center', fontsize=6, fontweight='bold')
        ...
        # Labels
        ax.text(2, 0.5, 'Gene Expression\n(Input)', ha='center', va='center', fontsize=8, fontweight='bold')
        ax.text(8, 0.5, 'Perturbation\n(Output)', ha='center', va='center', fontsize=8, fontweight='bold')
    ...
    def _draw_analysis_workflow(self, ax):
        ...
        # Analysis steps
        steps = [
            {'xy': (1, 4), 'size': (1.8, 1), 'label': 'Extract\nWeights', 'color': self.colors['model']},
            {'xy': (4, 4), 'size': (1.8, 1), 'label': 'Gene\nImportance', 'color': self.colors['analysis']},
            {'xy': (7, 4), 'size': (1.8, 1), 'label': 'Perturbation\nSimilarity', 'color': self.colors['analysis']},
            {'xy': (10, 4), 'size': (1.8, 1), 'label': 'Pathway\nEnrichment', 'color': self.colors['biology']},
            
            {'xy': (2.5, 1.5), 'size': (1.8, 1), 'label': 'Volcano\nPlots', 'color': self.colors['accent']},
            {'xy': (5.5, 1.5), 'size': (1.8, 1), 'label': 'Dotplots', 'color': self.colors['accent']},
            {'xy': (8.5, 1.5), 'size': (1.8, 1), 'label': 'Heatmaps', 'color': self.colors['accent']}
        ]
        
        for step in steps:
            rect = FancyBboxPatch(
                step['xy'], step['size'][0], step['size'][1],
                boxstyle="round,pad=0.05",
            ...
            ax.text(step['xy'][0] + step['size'][0]/2, step['xy'][1] + step['size'][1]/2,
        ...
        for x in [3.4, 6.4, 9.4]:
            ax.annotate('', xy=(x, 2.5), xytext=(x, 3.8),
    ...
    def _draw_example_results(self, ax1, ax2, model, adata):
        ...
        y_pos = np.arange(20)
        bars = ax1.barh(y_pos, gene_importance[top_20_idx], color=self.colors['analysis'], alpha=0.7)
        ...
        im = ax2.imshow(similarity_matrix, cmap='RdBu_r', vmin=-1, vmax=1, aspect='auto')
    ...
    def _draw_mock_results(self, ax):
        ...
        # Mock result boxes
        results = [
            {'xy': (0.5, 2.5), 'size': (2, 1), 'label': 'Gene\nImportance\nRanking', 'color': self.colors['analysis']},
            {'xy': (3, 2.5), 'size': (2, 1), 'label': 'Perturbation\nSimilarity\nMatrix', 'color': self.colors['analysis']},
            {'xy': (5.5, 2.5), 'size': (2, 1), 'label': 'Pathway\nEnrichment\nScores', 'color': self.colors['biology']},
            {'xy': (8, 2.5), 'size': (1.5, 1), 'label': 'Cell Type\nSignatures', 'color': self.colors['biology']},
            
            {'xy': (1.5, 0.5), 'size': (2, 1), 'label': 'Differential\nGene Analysis', 'color': self.colors['accent']},
            {'xy': (4, 0.5), 'size': (2, 1), 'label': 'Drug MoA\nPrediction', 'color': self.colors['accent']},
            {'xy': (6.5, 0.5), 'size': (2, 1), 'label': 'Biomarker\nDiscovery', 'color': self.colors['accent']}
        ]
        
        for result in results:
            rect = FancyBboxPatch(
                result['xy'], result['size'][0], result['size'][1],
                boxstyle="round,pad=0.05",
                facecolor=result['color'], alpha=0.6,
                edgecolor='black', linewidth=1
            )
            ax.add_patch(rect)
            
            ax.text(result['xy'][0] + result['size'][0]/2, result['xy'][1] + result['size'][1]/2,
                   result['label'], ha='center', va='center',
                   fontsize=7, fontweight='bold', color='white')
    ...
    def _draw_applications(self, ax):
        ...
        applications = [
            {'xy': (0.5, 1.5), 'size': (2.5, 1), 'label': 'Drug Discovery\n• MoA prediction\n• Target identification', 'color': self.colors['biology']},
            {'xy': (3.5, 1.5), 'size': (2.5, 1), 'label': 'Clinical Research\n• Biomarker discovery\n• Patient stratification', 'color': self.colors['analysis']},
            {'xy': (6.5, 1.5), 'size': (2.5, 1), 'label': 'Basic Biology\n• Pathway analysis\n• Gene regulation', 'color': self.colors['model']},
            {'xy': (9.5, 1.5), 'size': (2, 1), 'label': 'Method Development\n• Scalable analysis\n• Interpretability', 'color': self.colors['data']}
        ]
        
        for app in applications:
            rect = FancyBboxPatch(
                app['xy'], app['size'][0], app['size'][1],
                boxstyle="round,pad=0.05",
                facecolor=app['color'], alpha=0.6,
                edgecolor='black', linewidth=1
            )
            ax.add_patch(rect)
            
            ax.text(app['xy'][0] + app['size'][0]/2, app['xy'][1] + app['size'][1]/2,
                   app['label'], ha='center', va='center',
                   fontsize=7, fontweight='bold', color='white')
        
        # Add key advantages
        ax.text(6, 0.3, 'Key Advantages: Scalable • Interpretable • Fast • Memory-efficient', 
    ...
    def _add_panel_labels(self, fig, gs):
        ...
        for label, pos in zip(labels, positions):
            fig.text(pos[0], pos[1], label, fontsize=12, fontweight='bold',
