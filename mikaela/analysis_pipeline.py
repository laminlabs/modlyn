...
class CompleteAnalyzer:
    ...
    def create_weight_adata_for_scanpy(self, top_k=20):
        ...
        var_df = pd.DataFrame({
            'gene_name': top_gene_names,
            'original_index': top_gene_indices
        })
    ...
    def figure_2_scanpy_dotplot(self, top_k=25, **kwargs):
        ...
        try:
            ...
            dp = sc.pl.dotplot(
                ...
                size_title='|Weight|',
                figsize=(max(12, len(top_gene_names) * 0.4), max(6, len(weight_adata.obs) * 0.3)),
            ...
            self._create_custom_dotplot(weights_subset, top_genes, self.class_names[:n_classes_show])
            fig = plt.gcf()
    ...
    def _create_custom_dotplot(self, weights_subset, gene_names, class_names):
        ...
        fig, ax = plt.subplots(figsize=(max(12, len(gene_names) * 0.4), 
                                       max(8, len(class_names) * 0.3)))
        ...
        for i, class_name in enumerate(class_names):
            for j, gene_name in enumerate(gene_names):
                ...
                ax.scatter(j, i, s=size, c=norm_weight, cmap='RdBu_r', 
                          vmin=-2, vmax=2, alpha=0.8, edgecolors='black', linewidth=0.5)
        ...
        plt.colorbar(plt.cm.ScalarMappable(cmap='RdBu_r'), ax=ax, label='Normalized Weight')
        plt.tight_layout()
    ...
    def explore_perturbation_mechanisms(self):
        ...
        sns.heatmap(perturbation_similarity[:n_show, :n_show], 
        ...
        print(f"🔍 Found {len(similarity_pairs)} highly similar perturbation pairs (correlation > 0.7)")
    ...
    def explore_gene_networks(self, top_k=50):
        ...
        for cluster_id in unique_clusters:
            ...
            cluster_genes = [self.gene_names[top_genes_idx[i]] for i in np.where(mask)[0]]
    ...
    def analyze_confounders(self):
        ...
        technical_vars = [col for col in obs_cols if any(x in col.lower() 
                         for x in ['plate', 'batch', 'barcode', 'sample', 'well'])]
        
        biological_vars = [col for col in obs_cols if any(x in col.lower() 
...
"""
        ...
        if 'drug_similarity' in self.results:
            max_sim = np.max(self.results['drug_similarity'][self.results['drug_similarity'] < 0.99])
...
def run_complete_analysis(model, adata, save_prefix="analysis"):
    ...
    try:
        ...
        gene_importance = analyzer.figure_1_model_overview()
        ...
        perturbation_similarity = analyzer.explore_perturbation_mechanisms()
        gene_modules = analyzer.explore_gene_networks()
